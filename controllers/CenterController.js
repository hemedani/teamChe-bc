...
exports.addCenter =  ( req, res ) => {
  ...
  // let rastes = req.body.rastes || [];

  // let rastesRef = [];

  // Promise.all(rastes.map((EachRaste) => {
  //   return Raste.findOne({enName: EachRaste})
  //     .exec()
  //     .then((rasteFind) => {
  //       // let hasId = center.rastesRef.some(rasteRef => rasteRef.equals(rasteFind._id));
  //       // if (!hasId) center.rastesRef.push(rasteFind._id)
  //       return rastesRef.push(rasteFind._id)
  //     })
  // }))
  // .then((resp) => {
  //   center.rastesRef = rastesRef;
  //   center.save()
  //     .then((centerSaved) => {
  //       return res.json({ center: centerSaved });
  //     })
  //     .catch((err) => {
...
exports.fixCity = (req, res) => {
  City.findOne({ enName: 'Tehran' })
    ...
    .then((cityFind) => {
      if (cityFind) {
        Center.update({cityName: 'Tehran' }, { $set: { city: { location : cityFind.location, enName: 'Tehran', name: 'تهران' }}, cityRef: cityFind._id}, {multi: true})
        .exec()
        .then((resp) => {
...
exports.updateCenter = ( req, res, next ) => {
  ...
  Center.findOneAndUpdate({_id: req.body._id}, {
    name: req.body.name,
    enName: req.body.enName,
    cityName: req.body.city.enName,
    description: req.body.description,
...
exports.centers = (req, res, next) => {
  ...
  if (req.query.wareTypes) {
    let wareTypes = []
    Array.isArray(req.query.wareTypes) ? wareTypes = wareTypes.concat(req.query.wareTypes) : wareTypes.push(req.query.wareTypes)
  ...
  req.query.address ? Object.assign(query, {"address" : { "$regex": req.query.address }}) : query = query;
  ...
  Center.find(query)
    .limit(30)
    .sort({_id: -1})
    .select( 'name enName cityName workShift expertRate likes phone discount TotalQualityRate TotalPeopleRate TotalPriceRate TotalSalesmanRate options wareTypes rastes labels address premium onlineShop description pic location officeDoctors otherAdresses' )
    .exec()
...
exports.getCentersWithParams = function (req, res, next) {
  ...
  if (req.query.lat && req.query.lng) {
    ...
    (req.query.premium == 'true' || req.query.premium === true) ? query.premium = req.query.premium : query = query;
    (req.query.onlineShop == 'true' || req.query.onlineShop === true) ? query.onlineShop = req.query.onlineShop : query = query;
    ...
    Center.find(query)
      .skip(limit * page)
      .limit(limit)
      .select( 'name enName cityName workShift expertRate likes phone discount TotalQualityRate TotalPeopleRate TotalPriceRate TotalSalesmanRate options wareTypes rastes labels address premium onlineShop pic location officeDoctors otherAdresses' )
      .exec()
      .then((centers) => res.json({ centers }))
      .catch((err) => res.status(422).send({error: 'anjam neshod'}))
...
exports.center = (req, res, next) => {
  ...
  Center.findById(req.query.id)
    .populate('doctor')
    .select( 'name enName cityName description telegram instagram email website workShift expertRate likes phone discount TotalQualityRate TotalPeopleRate TotalPriceRate TotalSalesmanRate options wareTypes rastes labels address premium onlineShop pic staticMap location officeDoctors otherAdresses' )
    .exec()
    .then( (center) => {
      ...
      if (rateFind) {
        if (rateFind.qualityRate) {
          ...
          .then((centerFind) => {
            ...
            centerFind.TotalQualityRate.total = (centerFind.TotalQualityRate.total - rateFind.qualityRate) + req.body.qualityRate;

            if (centerFind.TotalPeopleRate.total) {
              centerFind.TotalPeopleRate.total = ((centerFind.TotalPeopleRate.total - rateFind.qualityRate) + req.body.qualityRate).toFixed(1);
            } else {
              centerFind.TotalPeopleRate.total = req.body.qualityRate;
              centerFind.TotalPeopleRate.count = 1;
            }

            centerFind.TotalQualityRate.average = (centerFind.TotalQualityRate.total / centerFind.TotalQualityRate.count).toFixed(1);
            centerFind.TotalPeopleRate.average = (centerFind.TotalPeopleRate.total / centerFind.TotalPeopleRate.count).toFixed(1);
      ...
      if (rateFind) {
        if (rateFind.priceRate) {
          ...
          .then((centerFind) => {
            ...
            if (centerFind.TotalPeopleRate.total) {
              centerFind.TotalPeopleRate.total = (centerFind.TotalPeopleRate.total - rateFind.priceRate) + req.body.priceRate;
            } else {
              centerFind.TotalPeopleRate.total = req.body.priceRate;
              centerFind.TotalPeopleRate.count = 1;
            }

            centerFind.TotalPriceRate.average = (centerFind.TotalPriceRate.total / centerFind.TotalPriceRate.count).toFixed(1);
            centerFind.TotalPeopleRate.average = (centerFind.TotalPeopleRate.total / centerFind.TotalPeopleRate.count).toFixed(1);
            centerFind.markModified(centerFind.TotalPeopleRate);
      ...
      if (rateFind) {
        if (rateFind.salesmanRate) {
          ...
          .then((centerFind) => {
            ...
            centerFind.TotalSalesmanRate.total = (centerFind.TotalSalesmanRate.total - rateFind.salesmanRate) + req.body.salesmanRate;

            if (centerFind.TotalPeopleRate.total) {
              centerFind.TotalPeopleRate.total = (centerFind.TotalPeopleRate.total - rateFind.salesmanRate) + req.body.salesmanRate;
            } else {
              centerFind.TotalPeopleRate.total = req.body.salesmanRate;
              centerFind.TotalPeopleRate.count = 1;
            }

            centerFind.TotalSalesmanRate.average = (centerFind.TotalSalesmanRate.total / centerFind.TotalSalesmanRate.count).toFixed(1);
            centerFind.TotalPeopleRate.average = (centerFind.TotalPeopleRate.total / centerFind.TotalPeopleRate.count).toFixed(1);
            centerFind.markModified(centerFind.TotalPeopleRate);
...
exports.addOptions = (req, res, next) => {
  ...
  let optionsEnName = [], optionsRef = [], options = req.body.options;
  ...
  Center.findByIdAndUpdate({_id: req.body.id}, {
    options: options,
    optionsEnName: optionsEnName,
    optionsRef: optionsRef,
  }, {new: true})
...
exports.setOtherAddresses = (req, res, next) => {
  Center.findByIdAndUpdate({_id: req.body._id}, { $push: {otherAdresses: {
...
exports.setOfficeDoctors = (req, res, next) => {
  ...
  let officeDoctors = req.body.officeDoctors || [], officeDoctorsRef = [];
  officeDoctors.map(ofdrs => officeDoctorsRef.push(ofdrs._id));

  Center.findByIdAndUpdate({_id: req.body.centerId}, { $push : { officeDoctors: {$each: officeDoctors}, officeDoctorsRef: {$each: officeDoctorsRef} } })
    .exec()
    .then((centerUpdated) => {
      Center.findById(req.body.centerId)
        .populate('doctor')
  ...
  if (hasLiked) {
    User.findByIdAndUpdate(req.user._id, { $pull: { centersLiked: centerId }}, {new: true})
      ...
      .then((userUpd) => {
        ...
        Center.findByIdAndUpdate(centerId, { $inc: { likes: -1 }}, {new: true})
          .then((centerUpd) => {
...
exports.addPicToCenter = (req, res, next) => {
  ...
  Center.findById(req.body._id)
    ...
    .then((centerFind) => {
      ...
      let picsUploaded = req.body.picsUploaded || [], pic = [], picRef = [], oldPic = centerFind.pic || [], oldPicRef = centerFind.picRef || [];
    ...
    .then((centersFind) => {
      ...
      Promise.all(centersFind.map((center) => {
        ...
        // if (center.officeDoctorsRef.length > 0) {
          ...
          return Promise.all(center.officeDoctorsRef.map((EachOfDr) => {
          ...
          .then((resp) => {
            ...
            center.officeDoctors = resp.map(ofdr => ({ pic: ofdr.pic, name: ofdr.name, familyName: ofdr.familyName, _id: ofdr._id }))
            return center.save()
              .then((centerSaved) => {
                return centerSaved
              })
...
exports.fixedStaticMaps = async (req, res) => {
  ...
  const centers = await Center.find().skip(limit * page).limit(limit).exec().catch((err) => res.status(422).send({error: 'anjam neshod', err}))

  const cnmap = await Promise.all( centers.map( async (center) => {
    ...
    const staticMapImgName = `${uuidv4()}.png`
        ...
        const downloadedMap = await download.image(mapOptions).catch((err) => res.status(422).send({error: 'anjam neshod', err}));
        center.staticMap = staticMapImgName;
...
exports.fixOtherAddressId = (req, res, next) => {
  ...
  Center.find()
    ...
    .then( async (centersFind) => {
      ...
      const centerFixed = await Promise.all(centerWithOtherAddress.map(centerWOAmap => {
        centerWOAmap.otherAdresses.map(EachOtherAdd => {
          EachOtherAdd._id = mongoose.Types.ObjectId()
        })
        return centerWOAmap.save()
          .then(centerWOAmapSaved => centerWOAmapSaved)
      }))
